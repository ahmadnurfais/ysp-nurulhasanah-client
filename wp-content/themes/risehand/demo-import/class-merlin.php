...
class Merlin {
	...
	protected function welcome() {
		...
		<?php $theme_directory_url = get_template_directory_uri(); 
			// Specify the path to the image within your theme
			$image_path = $theme_directory_url . '/demo-import/assets/images/welcome1.png'; 
			echo '<img src="' . esc_url($image_path) . '" alt="Your Image" />'; ?>
			<?php // echo wp_kses( $this->svg( array( 'icon' => 'done' ) ), $this->svg_allowed_html() ); ?>

			<h1><?php echo esc_html( sprintf( $header, $theme ) ); ?></h1>

			<p class="blok"><?php echo esc_html( sprintf( $paragraph, $theme ) ); ?></p>
			...
			<div class="merlin__select-control-wrapper"> 
				<select class="merlin__select-control js-merlin-demo-import-select">
					<?php foreach ( $this->import_files as $index => $import_file ) : ?>
						<option value="<?php echo esc_attr( $index ); ?>" data-preview-image="<?php echo esc_url( $import_file['import_preview_image_url'] ); ?>">
				...
				<script>
					document.addEventListener('DOMContentLoaded', function() {
						var selectElement = document.querySelector('.js-merlin-demo-import-select');
						var previewImage = document.getElementById('preview-image');

						function showPreviewImage() {
							var selectedOption = selectElement.options[selectElement.selectedIndex];
							var previewImageUrl = selectedOption.getAttribute('data-preview-image');
						...
						// Trigger change event when the page loads to show the preview image of the first selected option
						showPreviewImage();

						// Add change event listener to update the preview image when an option is selected
						selectElement.addEventListener('change', showPreviewImage);
					});
				</script> 
